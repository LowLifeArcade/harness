const mutatingWrangler = /\bwrangler\s+(deploy|delete|rollback|secret\s+(put|delete|bulk)|kv\s+.*\b(put|delete|create)|r2\s+.*\b(put|delete|create)|d1\s+.*\b(create|delete|execute|migrations\s+apply)|queues?\s+.*\b(create|delete)|vectorize\s+.*\b(create|delete|insert)|pages\s+deploy|versions?\s+deploy)\b/i
const destructive = [
  /(^|[;&|]\s*)rm\s+-[^\n]*r[^\n]*f\s+(\/|~|\$HOME)(\s|$)/i,
