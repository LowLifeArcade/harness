...
function toolLabel(type: string): string {
    return (
        ({ command_execution: 'SHELL', file_change: 'PATCH', mcp_tool_call: 'SKILL', web_search: 'LOOKUP' } as Record<string, string>)[
            type
...
function lastUsefulLine(stderr: string): string {
    return (
        stderr
            .split('\n')
            .map((line) => line.trim())
            .filter((line) => line && !line.includes(' WARN ') && !line.includes(' ERROR codex_models_manager::cache'))
            .at(-1) ?? ''
    );
}
...
function friendlyError(message: string): Error {
    if (message.includes('requires a newer version of Codex'))
        return new Error(
            `Model ${process.env.LOWLIFE_MODEL ?? 'gpt-5.4'} is not supported by this Codex CLI. Upgrade Codex or set LOWLIFE_MODEL=gpt-5.4.`,
        );
    if (message.includes('login') || message.includes('401') || message.includes('unauthorized'))
        return new Error('Codex is not signed in. Run `codex login` in your shell, then restart LowLifeArcade.');
    if (message.includes('ENOENT') || message.includes('not found'))
        return new Error('The Codex CLI is not installed. Install it, run `codex login`, then restart LowLifeArcade.');
