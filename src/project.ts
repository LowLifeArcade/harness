...
export const DATA_OPTIONS = [
  ...
  { name: "Durable Objects", value: "durable-objects", description: "Strongly consistent state for rooms, multiplayer, and realtime coordination." },
] as const
...
export async function createProject(baseDirectory: string, options: ProjectOptions, events: ProjectEvents): Promise<ProjectResult> {
  ...
  if (root !== baseDirectory && !root.startsWith(`${resolve(baseDirectory)}${sep}`)) throw new Error("Project path escapes the workspace.")
...
interface PackageShape { scripts?: Record<string, string> }
