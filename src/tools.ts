...
export const tools: FunctionTool[] = [
  {
    type: "function",
    name: "load_skill",
    description: "Load one installed Cloudflare skill before using its specialized guidance. Load wrangler before Wrangler commands and workers-best-practices before Worker code/config changes.",
    strict: true,
    parameters: {
      type: "object",
  ...
  },
  {
    type: "function",
    name: "run_command",
    description: "Run one shell command in the project. Read-only inspection and dry-runs are automatic; deploys, remote mutations, and other commands require user approval. Prefer Bun and bunx wrangler.",
    strict: true,
    parameters: {
      type: "object",
...
export async function executeTool(name: string, args: ToolArgs, context: ToolContext): Promise<string> {
  ...
  if (name === "read_file") {
    try {
      ...
      if (file.size > 100_000) return JSON.stringify({ error: "File is larger than 100 KB; inspect a smaller range with run_command." })
...
function processEnvWithoutSecrets(): Record<string, string> {
  const allowed = ["PATH", "SHELL", "TERM", "COLORTERM", "LANG", "LC_ALL", "TMPDIR", "USER", "LOGNAME", "HOME", "XDG_CONFIG_HOME", "CLOUDFLARE_ACCOUNT_ID"]
